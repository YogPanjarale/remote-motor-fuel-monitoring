...
const APIhandler: NextApiHandler = async (req, res) => {
  ...
  |> yield(name: "${func} of ${field}")
	...
	Promise.all([
        data.engine.engineRpm.min =await qb("rpm","min") ,
        data.engine.engineRpm.max = await qb("rpm","max"),
        data.engine.engineTemp1.min = await qb("temp1","min"),
        data.engine.engineTemp1.max = await qb("temp1","max"),
        data.engine.engineTemp2.min = await qb("temp2","min"),
        data.engine.engineTemp2.max =  await qb("temp2","max"),
        data.engine.engineTemp3.min = await qb("temp3","min"),
        data.engine.engineTemp3.max = await qb("temp3","max"),
        data.engine.engineTemp4.min =   await qb("temp4","min"),
        data.engine.engineTemp4.max = await qb("temp4","max"),
        data.engine.lubeOilPressure.min = await qb("lube_oil_pressure","min"),
        data.engine.lubeOilPressure.max = await qb("lube_oil_pressure","max"),
        // data.engine.engineRunningHours = await q(queryBuilder("engine_running_hours","sum")),
        data.fuelLevel.min = await qb("fuel_sensor_level","min"),
        data.fuelLevel.max = await qb("fuel_sensor_level","max"),
        data.waterFlowVolumeRate.min = await qb("water_flow_volume_rate_pulse","min"),
        data.waterFlowVolumeRate.max = await qb("water_flow_volume_rate_pulse","max"),
