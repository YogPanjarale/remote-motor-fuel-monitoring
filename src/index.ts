...
app.get("/devices/:id/create", async (req, res) => {
	if (req.body.apiKey !== apiKey) {
		res.status(403).json({
			...
			message: "api key required to create device",
		});
	}
	if (req.params.id) {
...
app.get("/devices/:id/engine/start", async (req, res) => {
	const device = await DeviceModel.findOne({ id: req.params.id });
	if (device) {
		events.emit("engine:start", req.params.id);
		return res.json({ status: "success", message: "engine started" });
	} else {
		return res.json({ status: "error", error: "device not found" });
	}
});
app.get("/devices/:id/engine/stop", async (req, res) => {
	const device = await DeviceModel.findOne({ id: req.params.id });
	if (device) {
		events.emit("engine:stop", req.params.id);
		return res.json({ status: "success", message: "engine stopped" });
	} else {
		return res.json({ status: "error", error: "device not found" });
	}
});
